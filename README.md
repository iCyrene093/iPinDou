...
- 导入图片后会按当前设置自动刷新图纸；调整宽或高会在尺寸变化时自动刷新图纸，刷新完成前会暂时禁止保存；“保存图纸”会保存当前屏幕上的图纸，保留手动编辑和镜像结果。
- 实时统计图纸尺寸、总豆数和各颜色用量。

## 项目结构

- `app/src/main/java/com/ipindou/app/MainActivity.java`：主界面、图片导入、生成、镜像、保存与统计。
...
如果当前机器没有可用 Android SDK platform，或无法获得 Android Gradle Plugin，`:app:assembleRelease` 才会回退到仓库内的离线构建器，生成一个仅用于受限沙箱冒烟验证的极简 APK。也可以显式运行离线任务：
